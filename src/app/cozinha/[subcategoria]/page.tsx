...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  const sub = categories.find(c => c.slug === 'cozinha')?.subcategories.find(s => s.slug === subcategoria);
  if (!sub) return {};
  return { title: sub.name + ' — Cozinha', description: sub.description, alternates: { canonical: '/cozinha/' + subcategoria }, openGraph: { title: sub.name + ' — Cozinha' } };
}

export function generateStaticParams() {
  return (categories.find(c => c.slug === 'cozinha')?.subcategories ?? []).map(s => ({ subcategoria: s.slug }));
}
    ...
    <>
      <section className="relative bg-teka-dark py-12 lg:py-20">
        ...
        <div className="relative max-w-7xl mx-auto px-6">
          <span className="text-teka-red text-sm font-semibold uppercase tracking-wider">Cozinha</span>
      ...
      <div className="max-w-7xl mx-auto px-6">
        <Breadcrumbs items={[{ label: 'Cozinha', href: '/cozinha' }, { label: sub.name }]} />
